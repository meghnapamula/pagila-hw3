 ...
 */
SELECT * FROM (SELECT actor_id, first_name, last_name, film_id, title, RANK() OVER (PARTITION BY actor_id ORDER BY sum(amount) DESC, film_id) AS rank, sum(amount) AS revenue FROM actor a JOIN film_actor fa USING (actor_id) JOIN film USING (film_id) JOIN inventory USING (film_id) JOIN rental USING (inventory_id) JOIN payment USING (rental_id) GROUP BY 1,2,3,4,5 ORDER BY 1,6) a WHERE rank <= 3;
